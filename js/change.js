...
function sSort(i, ii) { // По картинке (возрастание)
 if (i[0] > ii[0])
 return 1;
 else 
 if (i[0] <= ii[0])
...
function numbimg(i)
{
  ...
  var j=0;
  var cmp=''; 
  while (str.charAt(j)!='.') {cmp=cmp+str.charAt(j); j++;}
  if (cmp==i) return k;
  k++;  
  }
  return -1;
          ...
          while (countDet < colbolts-1) {
            if (data[countDet][isinrec-2]=='1') {
               ...
               for (j = 2; j < tdList.length - 2; j++) {
                  ...
                  if (data[countDet][j]!="")  ss = ss + data[countDet][j] + ",  ";
                   else ss = ss + data[countDet][j] ;
               }
...
function setmap(indx) {
 ...
 for (var ii = 0; ii < allSubgrGrData.length; ii++)
      ...
      while(kl<coords[indx].length)      
      {
         ...
         {
            if (posar[nn]==coords[indx][0]) { r=false; break;}
            nn++;         
         } 
         var note='';
           var v = 5;
         ...
         if (r) f = f + "<area alt='detal' title='" + note + "' id= 'area" + indx + "_" + kl + "' shape='rect' coords='" + coords[indx][kl][1] + "," + coords[indx][kl][2] + "," + coords[indx][kl][3] + "," + coords[indx][kl][4] + "' href='javascript:boxVisible(" + indx + "," + kl + ",true,true);' onmouseover='javascript:boxVisible(" + indx + "," + kl + ",true,false);'   onmouseout='javascript:boxVisible(-1,-1,true,true);boxVisible(curimg, curpos,true,false);'>";
        kl++;
       }    
...
function setToolTip(numbdetal,note)
{
  //  if (data[numbdetal - 1][1] != '') {
        ...
        if (aa >= data.length) aa = data.length;
   ...
   { 
  	...
  	var msg = "Добавлен в корзину";
		...
		m=allSubgrGrData[t].length-1;
             	if (n<m) {ss=ss + "  " + allSubgrGrData[t][n][3]+", " ; data[grCoher[ii][j]-1][isinrec-2] = '1';data[grCoher[ii][j]-1][isinrec-1] = val;  break;}else n-=m;       
		}    
              }
              //alert(ss);
             }         
...
function isNumberlnput (field, event)
{
   ...
   if (window.event)
	...
	if (event)
		key = event.which; //else return true;
    // Проверка на числа
...
function changeD(id) {
    ...
    if (preli) preli.style.backgroundColor = "#FFFFFF";
    previd = "li" + id;
...
function findIt(findtype) {
  var t=0;
    ...
    if (prevfind!=findv || cur!=curfind)
      ...
      {curfind=0;isfind=false; findIt(findtype);}
    }    
    else
    {
     if (prevfind!=findv || cur!=curfind)
      {prevfind=findv;countDet=data.length-1;} 
      if  (curfind!=0) 
      while (countDet >= 0) 
      {
       //for (var j=0;j<8;j++)  
 ...
 { curfind = data.length - 1; isfind = false; findIt(findtype); }
...
function open_tree(codegr, codeupgr) {
    ...
    for (var i = 0; i < grClassify.length; i++) {
        if (codeupgr == grClassify[i][0]) {
            ...
            if (grClassify[i][1] == 0) return;
            else open_tree(grClassify[i][0], grClassify[i][1]);
        }
...
function change(current, area_index, img_index, p,pp ) {
  ...
  if (current > 0) {
   ...
   group.innerHTML = "<div class=\"hh2\">"+ trans[0] +" / "+ trans[1] +":</div>" + "<div class=\"hh3\">" + data[cur][tablelenth - 1] + " / " + data[cur][tablelenth] + "</div>";//<img src=\"./img/recucle.jpg\" onclick=\"setCheck();\" id=\"check\">";
  /* for (j = 0; j < tablelenth - 2; j++) {
       if (tdListName[j].innerText == trans[0])
           gr = j;
       if (tdListName[j].innerText == trans[1] )
 ...
 for (var k = a; k <= aa; k++) {
     if (data[k - 1][0] == data[current - 1][0] && data[k - 1][1] == data[current - 1][1]) {
         dettorec[detindx] = k - 1;detsort[detindx] = k - 1;  detindx++;
         for (var m = 0; m < tablelenth; m++) {
             if (m < tablelenth - 2)
                  ...
                  for (var q1=0;q1<allSubgrGrNames[q].length;q1++)
                        {
                             ...
                             if (h>current-1)
                             {
			                    ...
			                    var aa=current - (h - allSubgrGrData[t].length+1);
 				                ...
 				                if (img) img.src = "./img/minus.gif";


 				                setcolored1('b' + c); open_tree(c, cl);
 				                scrollIV('b' + c, "ulclassify");
 				                } else {

 				                   if (prev1!=-1){ var s = document.getElementById(prev1);
 				                   if (s!=undefined)  
                                                   s.style.background = "#ffffff";}
 				                }
 				                if (data[current - 1][1] == '')
 				                {tree_item_opened=false; boxVisible(-1, -1,  true, true); }
 				                doResizeCode();
                                return;
                             }
                             t++;
...
if (imageNum>newCoords.length) 
  ...
  // 1) -   2) _| 3) _   4) |_
  var plus = new Array(
    new Array(p.x+c[0], p.y+c[1], c[2]-c[0], 4),
    new Array(p.x+c[2], p.y+c[1], 4, c[3]-c[1]),
    new Array(p.x+c[0], p.y+c[3], c[2]-c[0]+4, 4),
    new Array(p.x+c[0], p.y+c[1], 4, c[3]-c[1]+4)
  );
...
function clear_sel() {
    ...
    if (k!=undefined) k.style.background = "#ffffff";
...
function OpenImagePopup(imgPath, title, alt,name) {
   ...
   var cnt =grIllustration[cur_Ill].length-1;
    ...
    if (imgPath==grIllustration[j][1]) cur_Ill=j;
    cnt =grIllustration[cur_Ill].length-1;
    var win = window.open('','displayWindow',
    'width=650,height=480,left=0,top=0,screenX=0,screenY=0,resizable=1,scrollbar=0,status=0');
...
content +='; var addIll=[';
 for(var a=1;a< grIllustration[cur_Ill].length;a++)
    if (a==1)  content =content+ "\""+grIllustration[cur_Ill][a]+"\"";
else content =content+','+"\""+grIllustration[cur_Ill][a]+"\"";
content+=']; ';


content +='var addIllnames=[';
 for(var a=0;a< grIllustrationNames[cur_Ill].length;a++)
    if (a==0)  content =content+ "\""+ grIllustrationNames[cur_Ill][a]+"\"";
else content =content+','+"\""+grIllustrationNames[cur_Ill][a]+"\"";
content+=']; ';
...
{
 cur_Ill_index++;
if (addIll.length-1<cur_Ill_index) cur_Ill_index=0;
var image=document.getElementById("image");
image.src='./illustration/'+addIll[cur_Ill_index];
setcnt(cur_Ill_index+1);
}
...
{
 cur_Ill_index--;
 if (cur_Ill_index<0) cur_Ill_index=addIll.length-1;
var image=document.getElementById("image");
  image.src='./illustration/'+addIll[cur_Ill_index];
setcnt(cur_Ill_index+1);    
}
...
if (modify != 180)
    rec.style.height = countinrec * 30 + 100 + "px";//300 + "px";
else
...
var h2 = t.offsetHeight;
var ulsc = document.getElementById("ulclassify");
if (showcls2 == 0) ulsc.style.height = h1 + h2 + "px";
if (showcls2 == 0 && showcls == 1) ulsc.style.height = document.body.clientHeight - 160 + "px";
